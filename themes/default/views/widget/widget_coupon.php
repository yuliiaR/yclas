...
<form class=""  method="post" action="<?=URL::current()?>">         
    <?if (Model_Coupon::current()->loaded()):?>
        ...
        <p>
            <?=__('Discount off')?> <?=(Model_Coupon::current()->discount_amount==0)?round(Model_Coupon::current()->discount_percentage,0).'%':round(Model_Coupon::current()->discount_amount,0)?> <br>
