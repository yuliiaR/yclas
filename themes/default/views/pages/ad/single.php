...
<?else:?>
    <?=Form::errors()?>
    <div <?=$ad->structured_data()->displayScope()?>>
        <div class="page-header">
            ...
            <h1><?=$ad->structured_data()->content($ad->title)->property('name')->display()?></h1>
        </div>
        ...
        <div class="well ">
            <?if ($ad->price>0):?>
                <span <?=$ad->structured_data()->property('offers')->fallback('Offer', NULL)->display();?>>
                    <?=$ad->structured_data()->setType('Offer')->content(NULL, $ad->price)->property('price')->display('meta', TRUE)?>
                    <?=$ad->structured_data()->setType('Offer')->content(NULL, i18n::get_intl_currency_symbol())->property('priceCurrency')->display('meta', TRUE)?>
                    <?if(core::config('payment.stock') AND $ad->stock > 0):?>
                        <?=$ad->structured_data()->setType('Offer')->content(NULL, 'http://schema.org/InStock')->property('availability')->display('meta', TRUE)?>
                    <?endif?>
        ...
        <div>
            <?if(core::config('advertisement.description')!=FALSE):?>
              <div class="text-description"><?=$ad->structured_data()->content(Text::bb2html($ad->description,TRUE))->property('description')->display()?></div>
            <?endif?>
