...
<?else:?>
    ...
    <?if (core::config('advertisement.map')==1 AND $ad->latitude AND $ad->longitude):?>
        <p><img class="img-responsive" src="//maps.googleapis.com/maps/api/staticmap?center=<?=$ad->latitude?>&zoom=<?=Core::config('advertisement.map_zoom')?>&scale=false&size=600x300&maptype=roadmap&format=png&visual_refresh=true&markers=size:large%7Ccolor:red%7Clabel:·%7C<?=$ad->latitude?>,<?=$ad->longitude?>" alt="<?=$ad->title?> <?=__('Map')?>" style="width:100%;"></p>
