			...
			<div class="my_ad_item" id="fi<?=$favorite->id_favorite?>">
				<div class="my_ad_body clearfix">
					<div class="ad_pcoll">
						...
						<?else:?>
							<img data-src="holder.js/<?=core::config('image.width_thumb')?>x<?=core::config('image.height_thumb')?>?<?=str_replace('+', ' ', http_build_query(array('text' => $favorite->ad->category->name, 'size' => 14, 'auto' => 'yes')))?>" alt="<?=HTML::chars($favorite->ad->title)?>"> 
						<?endif?>
						</div>
					</div>
					<div class="ad_dcoll">
