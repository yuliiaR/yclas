...
class Controller_User_Auth extends Auth_Controller {
	...
	{	    
	    ...
	    $this->template->content = View::factory('pages/user/auth/login');
	}
	...
	public function action_logout()
	{
	    ...
	    $this->request->redirect(Route::url('user',array('directory'=>'user','controller'=>'auth','action'=>'login')));
	...
	{
		$this->template->content = View::factory('pages/user/auth/forgot');
		...
		elseif ($this->request->post('email') AND CSRF::valid('forgot'))
		{
			...
			if (Valid::email($email,TRUE))
			{
				...
				if ($user->loaded())
				{
					...
					$url_change = Route::url('user',array('directory'  => 'user',
														  'controller' => 'profile', 
														  'action'     => 'changepass'),'http');					
				
					$ql = Auth::instance()->ql_encode($user->token,$url_change);
					
					$url_ql = Route::url('user',array('directory'  => 'user',
													  'controller' => 'auth', 
													  'action'     => 'ql',
													  'id'		=>$ql),'http');
					//@todo not hard coded
	...
	{
		$this->template->content = View::factory('pages/user/auth/register');
		$this->template->content->msg = '';
	...
	public function action_ql()
	{
		...
		if ($url==FALSE)
		{
			$url = Route::url('user',array('directory'  => 'user',
										  'controller' => 'auth', 
										  'action'     => 'login'),'http');	
		}
